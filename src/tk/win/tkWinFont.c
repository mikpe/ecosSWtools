...
#if 0
    /* CYGNUS LOCAL: We don't need these.  */
				 ...
				 * terminating character. */
{
    ...
    /* CYGNUS LOCAL: That does not seem to be true in Windows NT 4.0.
       I changed the code to use GetTextExtentExPoint.  */
	...
	if ((flags & TK_WHOLE_WORDS) && max < numChars) {
	    ...
	    /* We need to back up through the word that fits at the
               end of the line, and then back through any space
               characters that may precede that word.  We start
               backing up at MAX, because if MAX is a space we can
               stop at MAX - 1.  */
	    i = max;
	    ...
	    /* We must have executed at least one --i, so this will
               not set MAX larger than the original value.  */
	    max = i + 1;
	}
