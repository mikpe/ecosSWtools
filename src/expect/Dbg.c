...
char *argv[];
{
	...
	while (argc && (space > 0)) {
		...
		if (proc && (arg_index > 1)) wrap = TRUE;
			...
			(void) TclFindElement(interp,*argv,strlen(*argv),&elementPtr,
					&nextPtr,(int *)0,(int *)0);
#else
...
#if TCL_MAJOR_VERSION > 7
		/* FIXME: Either need to convert to strings or modify the
		   routines to take Tcl_Obj lists. */
		print(interp,"%c%d: %s\n",ptr,curf->level,
			print_argv(interp,curf->objc,curf->objv));
#else
		print(interp,"%c%d: %s\n",ptr,curf->level,
...
Tcl_Interp *interp;
{
	...
	while (TRUE) {
		...
		if (newcmd) {
			print(interp,"dbg%d.%d> ",iPtr->numLevels,-1); /* FIXME */
		} else {
			print(interp,"dbg+> ");
		}
		fflush(stdout);
