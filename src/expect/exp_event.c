...
struct pipe_info {
    ...
    HANDLE handle;		/* The underlying win32 handle.  */
...
LPVOID arg;
{
	...
	while (1) {
		...
		WaitForSingleObject (pipe->try_read, INFINITE);
		...
		while (1) {
			if (PeekNamedPipe (pipe->handle, NULL, 0, NULL, &n,
					   NULL) == FALSE) {
				n = 0;
				break;
			}
		...
		if (n == 0) {
			/* PeekNamedPipe returned an error.  FIXME:
                           should we try to report the error?  It's
                           normal for ERROR_INVALID_HANDLE to arise
                           here, meaning that the pipe was closed by
                           the main program.  */
			break;
		}
		...
		/* We've indicate that the pipe is readable, so ignore
                   any requests to check.  */
		ResetEvent (pipe->try_read);
	...
	CloseHandle (pipe->flags_mutex);
	CloseHandle (pipe->try_read);
...
Tcl_Interp *interp;
{
	...
	if (pipe_get_flags (p) & PIPE_HAS_THREAD) {
		WaitForSingleObject (p->flags_mutex, INFINITE);
		p->flags |= PIPE_CLOSED;
		SetEvent (p->try_read);
		ReleaseMutex (p->flags_mutex);
	} else {
...
int mask;
{
	...
	if (p->watch_mask != 0) {
		...
		if ((p->watch_mask & TCL_READABLE) != 0
			...
			thread = CreateThread (NULL, 0, pipe_thread,
					       p, 0, &tid);
			CloseHandle (thread);
		}
...
ClientData cd;
{
	if (exp_fs[fd].channel == NULL) {
		...
		if (getsockname (fd, &sa, &salen) == 0)
			...
			chan = Tcl_MakeFileChannel ((ClientData) handle,
						    (TCL_READABLE
						     | TCL_WRITABLE));
		else {
