...
#define SPAWN_OUT "spawn_out"
		...
		/* This doesn't work on cygwin32, because
                   ...
                   handle, and that will blow up the exp_fs array,
                   which expects to be indexed by file descriptor
                   number.  This should be fixed by changing the array
                   to not be so closely tied to file descriptors.  */
		exp_error(interp,"spawn -open and -leaveopen not supported on cygwin32");
		return TCL_ERROR;
#endif
...
#if TCL_MAJOR_VERSION > 7
			Tcl_GetChannelHandle(chan, TCL_READABLE, &rfd);
#else
...
#if TCL_MAJOR_VERSION > 7
			Tcl_GetChannelHandle(chan, TCL_WRITABLE, &wfd);
#else
		...
		master = ((mode & TCL_READABLE)?(int)rfd:(int)wfd);
...
#endif
		restore_error_fd
		errorlog("failed to get controlling terminal using TIOCSCTTY: %s\n", Tcl_ErrnoMsg(errno));
		exit(-1);
	}
#endif
...
char **argv;
{
	...
	sprintf(interp->result,"%d",(int) getpid());
	return(TCL_OK);
}
...
#if TCL_MAJOR_VERSION > 7
		Tcl_GetChannelHandle(chan, TCL_WRITABLE, &tclWriteFile);
		fd = dup((int)tclWriteFile);
#else
		tclWriteFile = Tcl_GetChannelFile(chan, TCL_WRITABLE);
...
char **argv;
{
	...
	/*SUPPRESS 591*/
#if !defined (DO_SETSID) && !defined (SYSV3) && defined (TIOCNOTTY)
	int ttyfd;
#endif
...
#if TCL_MAJOR_VERSION > 7
	int result = Tcl_ReturnObjCmd(clientData,interp,argc,argv);
#else
...
struct exp_cmd_data *c;
{
	...
	for (;c->name;c++) {
		...
		if (create) {
#if TCL_MAJOR_VERSION > 7
			if (!(c->flags & EXP_USE_OBJCMD)) {
				Tcl_CreateCommand(interp,c->name,c->proc,
						  c->data,exp_deleteProc);
			}
			else {
				Tcl_CreateObjCommand(interp,c->name,c->proc,
						     c->data,exp_deleteProc);
			}
#else
			Tcl_CreateCommand(interp,c->name,c->proc,
...
{"inter_return",Exp_InterReturnCmd,	0,	EXP_USE_OBJCMD},
