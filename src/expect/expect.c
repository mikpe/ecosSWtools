...
char **argv;
{
	...
	for (a=buf;*a && a<term;) {
		char *b;
		for (;isspace(*a) && a < term;a++) {
			if (*a == '\n') *a = ' ';
		}
#if TCL_MAJOR_VERSION > 7
		b = TclWordEnd(a,term,0,(int *)0);
#else
		b = TclWordEnd(a,0,(int *)0);
#endif
		if (*b)
		  b++;
		a = b;
...
char *caller_name;
{
	...
	sprintf(spawn_id,"%d",(int) (f-exp_fs));
...
char *array;
{
	...
	sprintf(ascii,"%ld",(long int) *timeval);
	out("epoch");
...
/*		exp_strftime(interp->result,TCL_RESULT_SIZE,format,tm);*/
		...
		sprintf(interp->result,"%ld", (long int) seconds);
	}
