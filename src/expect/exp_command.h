...
struct exp_f {
	...
	int Master;		/* corresponds to master fd */
	int Slave;		/* corresponds to slave_fd */
	int MasterOutput;	/* corresponds to tcl_output */
#else
...
struct tm;
EXTERN void		exp_strftime _ANSI_ARGS_ ((char *, const struct tm *, Tcl_DString *));
...
#define EXP_USE_OBJCMD  4	/* Pass objects to the TCL command instead of
				   strings (used by TCL 8 only)*/
