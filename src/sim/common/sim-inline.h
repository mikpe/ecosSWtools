...
/* INLINE CODE SELECTION:
   ...
   declaration and definition using one of the following methods:
	  ...
	  .c: INLINE_OURPKG (void)
       ...
       GLOBAL NON-INLINE FUNCTIONS:
          ...
          however, include large rarely used functions that contribute
          little when inlined.

	  The current mechanism still adds these functions to all
	  including modules.  It just doesn't inline them.
	  
	  .h: EXTERN_OURPKG (int) ourpkg_print
	      (char *fmt, ...);
   ...
   NOW IT WORKS:
      ...
      Consequently, all the code in ourprog.c is visible and static.
      ...
      true so it defines *_OURPROG as static.
      ...
      The expression: ``defined (SIM_INLINE) && ! defined (OURPROG_C)
      && REVEAL_MODULE_P (OURPROG_INLINE)'' is false so it defines
      *_OURPROG as non-static.

      Consequently, ourprog.o will contain the single copy of all the
      exported symbols.
...
#  define EXTERN_SIM_MAIN(TYPE) static TYPE UNUSED REGPARM_SIM_MAIN
# else
...
# define EXTERN_SIM_MAIN(TYPE) TYPE REGPARM_SIM_MAIN
#endif
...
#  define EXTERN_ENGINE(TYPE) static TYPE UNUSED REGPARM_ENGINE
# else
...
# define EXTERN_ENGINE(TYPE) TYPE REGPARM_ENGINE
#endif
