...
main (int argc, char **argv)
{
  ...
  if (single_step)
      ...
      do
	{
	  ...
	  if(reason == sim_stopped && sigrc == SIGINT) break; /* exit on control-C */
	}
      while ((reason == sim_signalled && sigrc == SIGTRAP) /* remain on breakpoint */
	     || (reason == sim_stopped                     /* remian on signals in oe mode */
    ...
    } while((reason == sim_stopped) /* remain on signals in oe mode */
	    && (STATE_ENVIRONMENT (sd) == OPERATING_ENVIRONMENT));
