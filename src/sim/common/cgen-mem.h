...
{ \
  ...
  return XCONCAT2 (sim_core_read_unaligned_,size) (cpu, NULL_CIA, read_map, a); \
}
#else
#define DECLARE_GETMEM(mode, size) \
...
{ \
  ...
  /* Don't read anything into "unaligned" here.  Bad name choice.  */ \
  XCONCAT2 (sim_core_write_unaligned_,size) (cpu, NULL_CIA, write_map, a, val); \
}
#else
#define DECLARE_SETMEM(mode, size) \
...
{ \
  /*PROFILE_COUNT_READ (cpu, a, XCONCAT2 (MODE_,mode));*/ \
  /* Don't read anything into "unaligned" here.  Bad name choice.  */\
  return XCONCAT2 (sim_core_read_unaligned_,size) (cpu, NULL_CIA, exec_map, a); \
}
#else
#define DECLARE_GETIMEM(mode, size) \
