...
/* Simulator for the MIPS architecture.
   ...
   $Revision: 1.125.2.1.4.11 $
   $Date: 1998/12/03 10:09:47 $             
     ...
     char **argv;
{
  ...
  if (board != NULL
	  ...
	  strcmp(board, BOARD_JMR3904_DEBUG) == 0))
    {
      ...
      for (i=0; i<4; i++) /* 4 MB total */
	{
	  unsigned size = 1024 * 1024;  /* 1 MB */
                 ...
                 sim_core_signals sig)
{
  ...
  switch (sig)
    ...
    case sim_core_unmapped_signal:
      ...
      BADVADDR = addr; /* XXX: should be passed to SignalException...() instead */ 
