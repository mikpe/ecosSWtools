...
/* Set this for -gdwarf-2.  For now it only tells gas not to
   generate any debugging info (because the compiler has taken
   care of this). */
int mips_dwarf2_debug = 0;
     ...
     int x;
{
  ...
  if (!is_end_of_line[(unsigned char) *input_line_pointer])
    ...
    p = NULL;
  if (ECOFF_DEBUGGING && now_seg != text_section)
