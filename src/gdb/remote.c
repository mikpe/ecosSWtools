...
/* The length of data in the buffer for binary downloads .
   If the value is -1 and remote_binary_download is true,
   then putpkt behaves normally. */
static int remote_binary_length = -1;
     ...
     int len;
{
  ...
  while (len > 0)
	...
	{
          int escaped = 0;
          remote_binary_length = strlen (buf) + todo;
          for (i = 0; 
	       ...
	       i++) 
            {
              switch (myaddr[i] & 0xff)
                ...
                case '#':
                case 0x7d:
                  /* These must be escaped.  Since that adds an extra char
		     to the buffer, it may fill the buffer with fewer than
		     "todo" bytes in it.  */
		  escaped++;
		  *p++ = 0x7d;
	  ...
	  remote_binary_length += escaped - (todo - i);
	  if (i < todo)
	    {
      ...
      else
	{
	  ...
	  *p = '\0';
      ...
      putpkt (buf);
      getpkt (buf, 0);
      remote_binary_length = -1;

      if (remote_binary_download && buf[0] == '\0')
        {
          /*  The stub does not understand this packet, so
              disable it from being used. */
          remote_binary_download = 0;

          return remote_write_bytes (memaddr, myaddr, len);
        }
     ...
     char *buf;
{
  ...
  if (remote_binary_download && remote_binary_length != -1)
    cnt = remote_binary_length;
  else
    cnt = strlen (buf);
