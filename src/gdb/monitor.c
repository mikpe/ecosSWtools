...
void
monitor_dcache_init (readfun, writefun)
     int (*readfun)(), (*writefun)();
{
  if (readfun == NULL)
    readfun = monitor_read_memory;
  if (writefun == NULL)
    writefun = monitor_write_memory;

  remote_dcache = dcache_init (readfun, writefun);
}
