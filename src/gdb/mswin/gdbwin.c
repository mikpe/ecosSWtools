...
/* Grab disassembly by redirecting gdb's output using
   fputs_unfiltered_hook.  XXX not very robust.  probably
   should use the existing Credirect mechanism in iface.cpp */
...
static void
my_fputs_unfiltered (const char *data, FILE * stream)
{
  strcat (output_catcher.end, data);
}

/* Compare this with gdb_disassemble() in gdbtk.c */

int
togdb_disassemble (CORE_ADDR addr, char *buf)
{
  disassemble_info info;
  void (*old_fputs_unfiltered_hook) (const char *, FILE *);
  int val;

  output_catcher.end = buf;
  *buf = '\0';

  INIT_DISASSEMBLE_INFO (info, gdb_stdout,
			 (fprintf_ftype) fprintf_unfiltered);
  info.read_memory_func = dis_asm_read_memory;
  info.memory_error_func = dis_asm_memory_error;
  info.print_address_func = dis_asm_print_address;
  info.mach = tm_print_insn_info.mach;
