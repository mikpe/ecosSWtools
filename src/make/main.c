...
Copyright (C) 1988, 89, 90, 91, 94, 95, 96, 98 Free Software Foundation, Inc.
...
/* CYGNUS LOCAL: cygwin32 has two additional flags */
#ifdef __CYGWIN32__
    { 4, flag_off, (char *) &shell_mode, 1, 1, 0, 0, 0,
        "unix", 0,
        "Run in UNIX mode (use sh.exe subshell)" },
    { 5, flag, (char *) &shell_mode, 1, 1, 0, 0, 0,
        "win32", 0,
...
#endif
{
...
#ifdef __CYGWIN32__
  {
    ...
    /* default to Win32 mode */
    shell_mode = WIN32_SHELL;
    path_separator_char = ';';

    /* Read the environment variable MAKE_MODE */
    /* If it's "UNIX", set UNIX_SHELL mode.  If it's "WIN32" or
       anything else, stay with the default of WIN32_SHELL mode. */
    make_mode_env = getenv ("MAKE_MODE");
    if (make_mode_env)
      if (strcaseequ (make_mode_env, "UNIX"))
        {
          shell_mode = UNIX_SHELL;
          path_separator_char = ':';
        }
  }
#endif /* __CYGWIN32 */
...
/* CYGNUS LOCAL: or for cygwin32 */
  ...
  /* Now that the command line switches have been read in,
     ...
     of shell_mode since they might invoked make with --unix */
...
#define w32ify(s,r) (s)
...

  {
    ...
    /* CYGNUS LOCAL: cygwin32 */
    /* Set the appropriate value for default_shell based on the shell
       mode make is in. */
#ifdef __CYGWIN32__
    if (shell_mode == WIN32_SHELL)
      {
        /* pick the appropriate DOS shell based on the operating system */
        if (get_os_type() == win95)
          default_shell = "command.com";
        else
          default_shell = "cmd.exe";
      }
    else /* UNIX_SHELL mode */
